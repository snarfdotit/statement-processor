...
public enum ValidationQuery {
    DUPLICATE("select s.reference_number, s.description, 'DUB' as error  from customer_statement s"
    ...
    END_BALANCE("select reference_number, description, 'CALC' as error  from customer_statement where (start_balance + mutation - end_balance != 0) ");
